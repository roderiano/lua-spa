...
def run_quality_checks() -> tuple[int, int]:
    ...
    return ruff_code, ruff_format_check_code
...
def main() -> int:
    ruff_code, ruff_format_check_code = run_quality_checks()

    # Always format/fix when any check fails.
    if ruff_code != 0 or ruff_format_check_code != 0:
        print(
    ...
    return 0 if all(code == 0 for code in (ruff_code, ruff_format_check_code)) else 1
